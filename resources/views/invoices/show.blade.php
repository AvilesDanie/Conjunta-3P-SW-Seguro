                ...
                <div class="p-6">
                    ...
                    <div class="mb-6">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="text-right">
                                <span class="px-3 py-1 text-sm font-medium rounded-full    @if($invoice->payment && $invoice->payment->status === 'pagado')        bg-green-100 text-green-800    @elseif($invoice->payment && $invoice->payment->status === 'rechazado')        bg-red-100 text-red-800    @elseif($invoice->payment && $invoice->payment->status === 'pendiente')        bg-yellow-100 text-yellow-800    @else        bg-gray-100 text-gray-800    @endif">
                                    ...
                                    PENDIENTE
                                    @else
                                    {{ $invoice->status == 'cancelled' ? 'CANCELADA' : 'SIN PAGO' }}
                                    @endif
                                </span>
                    ...
                    <div class="mt-6 bg-blue-50 border border-blue-200 rounded-md p-4">
                        <div class="flex">
                            ...
                            <div>
                                <h3 class="text-sm font-semibold text-blue-800 mb-2">Información de Pago</h3>
                                <div class="text-sm text-blue-700 space-y-1">
                                    ...
                                    <p><strong>Método de Pago:</strong> {{ $invoice->payment->payment_method ?? 'No especificado' }}</p>
                                    <p><strong>Fecha de Pago:</strong>
                                        {{ $invoice->payment->paid_at ? $invoice->payment->paid_at->format('d/m/Y H:i') : 'No registrada' }}
                                    </p>

                                    <p><strong>Número de Transacción:</strong>
                                        {{ $invoice->payment->transaction_number ?? 'No registrado' }}
                                    </p>

                                    <p><strong>Monto Pagado:</strong>
                                        {{ $invoice->payment->amount ? 'S/ ' . number_format($invoice->payment->amount, 2) : 'No registrado' }}
                                    </p>

                                    @if($invoice->payment->observations)
                                    <p><strong>Observaciones:</strong> {{ $invoice->payment->observations }}</p>
                                    @endif

                                    @if(
                                        $invoice->payment &&
                                        auth()->user()->hasRole('Administrador') &&
                                        $invoice->payment->status === 'pendiente' &&
